...
?>
    ...
    <table class="table table-condensed">
        ...
        @elseif($status->status==1)
            ...
            @if($status->academic_program=="CBEAS" OR $status->academic_program=="CAMPS" )    
                <form class="form-horizontal" method="POST" action="{{url('registrar',array('main','selectsubjectcollege'))}}">    
                {{ csrf_field() }}
            ...
            @elseif($status->academic_program=="Senior High School")
             <form class="form-horizontal" method="POST" action="{{url('registrar',array('main','selectsubjectshs'))}}">    
                {{ csrf_field() }}
                <input type="hidden" name="academic_program" value="{{$status->academic_program}}">
                <input type="hidden" name="idno" value="{{$idno}}">
